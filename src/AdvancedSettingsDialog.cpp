...
AdvancedSettingsDialog::AdvancedSettingsDialog(QWidget * parent) : QDialog(parent), ui(new Ui::AdvancedSettingsDialog) {
    ui->setupUi(this);
}
...
void AdvancedSettingsDialog::on_aceStepBrowseButton_clicked() {
    QString dir =
        QFileDialog::getExistingDirectory(this, "Select AceStep Build Directory", ui->aceStepPathEdit->text());
    if (!dir.isEmpty()) {
...
void AdvancedSettingsDialog::on_qwen3BrowseButton_clicked() {
    QString file =
        QFileDialog::getOpenFileName(this, "Select Qwen3 Model", ui->qwen3ModelEdit->text(), "GGUF Files (*.gguf)");
    if (!file.isEmpty()) {
...
void AdvancedSettingsDialog::on_ditBrowseButton_clicked() {
    QString file =
        QFileDialog::getOpenFileName(this, "Select DiT Model", ui->ditModelEdit->text(), "GGUF Files (*.gguf)");
    if (!file.isEmpty()) {
